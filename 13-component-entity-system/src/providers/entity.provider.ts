...
export default class EntityProvider {
    public generateEntity(...componentClasses: IComponentType<IComponent>[]): IEntity {
    ...
    public hasComponent<T extends IComponent>(entity: IEntity, componentClass: IComponentType<T>): boolean {
    ...
    public getComponent<T extends IComponent>(entity: IEntity, componentClass: IComponentType<T>): T {
        const tag = componentClass.name;
        const component = entity.components[tag];
        if (!component) {
    ...
    public putComponent<T extends IComponent>(entity: IEntity, componentClass: IComponentType<T>): T {
        const tag = componentClass.name;
        const component = entity.components[tag];
        if (component) {
            if (!this.cast(component, componentClass)) {
    ...
    public removeComponent<T extends IComponent>(entity: IEntity, componentClass: IComponentType<T>) {
        const tag = componentClass.name;
        const component = entity.components[tag];
        if (!component) {
